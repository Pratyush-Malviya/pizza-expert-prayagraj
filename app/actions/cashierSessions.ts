...
export async function getShiftSummary(shiftId: string) {
  try {
    await requireUser(['cashier', 'manager', 'super_admin', 'accountant', 'admin'])
    const supabase = createAdminClient()
    ...
    return { success: false, error: err.message, summary: null, shift: null, movements: [] }
  }
}
...
export async function closeCashierShift(
  ...
  try {
    ...
    if (!summary.success || !summary.summary) return { success: false, error: 'Could not load shift summary' }
